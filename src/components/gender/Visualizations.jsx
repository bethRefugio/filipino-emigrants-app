...
export default function Visualizations({ data, selectedCategories }) {
  ...
  const chartData = years.map((year) => {
    ...
    Object.keys(GENDER_LABELS).forEach((cat) => {
  ...
  const grandTotals = Object.keys(GENDER_LABELS).reduce((acc, cat) => {
  ...
  return (
    <div className="mt-0.2 px-6 pb-10">
      <h3 className="text-lg font-semibold mb-4">Gender Visualizations</h3>
            ...
            <PieChart>
              ...
              >
                {donutData.map((entry) => (
                  <Cell
                    key={entry.key}
                    fill={GENDER_COLORS[entry.key] || "#8884d8"}
                  />
                ))}
              </Pie>
              <Tooltip />
            ...
            <LineChart data={chartData}>
              ...
              {visibleCategories.map((cat) => (
                <Line
                  ...
                  type="monotone"
                  dataKey={cat}
                  stroke={GENDER_COLORS[cat] || "#8884d8"}
                  name={GENDER_LABELS[cat]}
                  strokeWidth={2}
                  dot={false}
