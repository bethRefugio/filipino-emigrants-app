...
export default function TopNavbar({
  ...
  // "All" checkbox logic
  const allChecked = selectedCategories.length === ageCategories.length;
  const handleAllChange = () => {
    if (allChecked) setSelectedCategories([]);
    else setSelectedCategories(ageCategories.map(c => c.key));
  };

  return (
    <div className="bg-white shadow-sm p-4 flex items-center justify-between sticky top-0 z-10">
      <div>
        <h2 className="text-2xl font-bold text-gray-800">Filipino Emigrants - Age</h2>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-56 bg-white border rounded-lg shadow-lg z-50 p-4">
              <div className="font-semibold text-xs mb-2">Filter by Age Group</div>
