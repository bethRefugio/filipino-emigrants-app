...
export default function originTable({
  ...
  const allColumns = [
    ...originNames.map(name => ({ key: name, label: name }))
  ];

  // Only show checked columns
  const visibleColumns = allColumns.filter(col => selectedProvinces.includes(col.key));
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800">
            Emigrants by Province ({sortedData.length})
          </h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {paginatedData.map((e) => {
                // Only sum visible provinces for the total
