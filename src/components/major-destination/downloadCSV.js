...
import { addEmigrant } from './services/major-destination';
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (values.length >= headers.length) {
            const record = {
              ...
              usa: Number(values[headers.indexOf('usa')]) || 0,
              canada: Number(values[headers.indexOf('canada')]) || 0,
              japan: Number(values[headers.indexOf('japan')]) || 0,
              australia: Number(values[headers.indexOf('australia')]) || 0,
              italy: Number(values[headers.indexOf('italy')]) || 0,
              new_zealand: Number(values[headers.indexOf('new_zealand')]) || 0,
              united_kingdom: Number(values[headers.indexOf('united_kingdom')]) || 0,
              germany: Number(values[headers.indexOf('germany')]) || 0,
              south_korea: Number(values[headers.indexOf('south_korea')]) || 0,
              spain: Number(values[headers.indexOf('spain')]) || 0,
              others: Number(values[headers.indexOf('others')]) || 0,
            };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <code className="block bg-white p-3 rounded text-xs text-gray-800 overflow-x-auto">
              year,usa,canada,japan,australia,italy,new_zealand,united_kingdom,germany,south_korea,spain,others
            </code>
            <p className="text-xs text-blue-600 mt-2">
              Example: 2024,1200,450,2100,2600,1950,2400,780,940,1580,1850
            </p>
          </div>
            ...
            <button
              onClick={() => {
                const sampleCSV = `year, usa, canada, japan, australia, italy, new_zealand, united_kingdom, germany, south_korea, spain, others
                                    2024,1200,450,2100,2600,1950,2400,780,940,1580,1850,430
                                    2023,1100,420,2050,2500,1900,2350,750,910,1520,1800,410`;
                const blob = new Blob([sampleCSV], { type: 'text/csv' });
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'major-country_data_sample.csv';
                a.click();
